<?php
require '../helpers.php';

require basePath('views/home.view.php');
?>

<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8" />
    <meta http-equiv="X-UA-Compatible" content="IE=edge" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.2/css/all.min.css" integrity="sha512-z3gLpd7yknf1YoNbCzqRKc4qyor8gaKU1qmn+CShxbuBusANI9QpRohGBreCFkKxLhei6S9CQXFEbbKuqLg0DA==" crossorigin="anonymous" referrerpolicy="no-referrer" />
    <link rel="stylesheet" href="css/style.css" />
    <title>Workopia</title>
</head>

<body class="bg-gray-100">
    <!-- Nav -->
    <header class="bg-blue-900 p-4 text-white">
        <div class="flex justify-between items-center mx-auto container">
            <h1 class="font-semibold text-3xl">
                <a href="index.html">Workopia</a>
            </h1>
            <nav class="space-x-4">
                <a href="login.html" class="text-white hover:underline">Login</a>
                <a href="register.html" class="text-white hover:underline">Register</a>
                <a
                    href="post-job.html"
                    class="bg-yellow-500 hover:bg-yellow-600 hover:shadow-md px-4 py-2 rounded text-black transition duration-300"><i class="fa fa-edit"></i> Post a Job</a>
            </nav>
        </div>
    </header>

    <!-- Showcase -->
    <section
        class="relative flex items-center bg-cover bg-no-repeat bg-center h-72 showcase">
        <div class="overlay"></div>
        <div class="z-10 mx-auto text-center container">
            <h2 class="mb-4 font-bold text-white text-4xl">Find Your Dream Job</h2>
            <form class="block mx-5 md:mx-auto mb-4">
                <input
                    type="text"
                    name="keywords"
                    placeholder="Keywords"
                    class="mb-2 px-4 py-2 focus:outline-none w-full md:w-auto" />
                <input
                    type="text"
                    name="location"
                    placeholder="Location"
                    class="mb-2 px-4 py-2 focus:outline-none w-full md:w-auto" />
                <button
                    class="bg-blue-500 hover:bg-blue-600 px-4 py-2 focus:outline-none w-full md:w-auto text-white">
                    <i class="fa fa-search"></i> Search
                </button>
            </form>
        </div>
    </section>

    <!-- Top Banner -->
    <section class="bg-blue-900 py-6 text-white text-center">
        <div class="mx-auto container">
            <h2 class="font-semibold text-3xl">Unlock Your Career Potential</h2>
            <p class="mt-2 text-lg">
                Discover the perfect job opportunity for you.
            </p>
        </div>
    </section>

    <!-- Job Listings -->
    <section>
        <div class="mx-auto mt-4 p-4 container">
            <div class="mb-4 p-3 border border-gray-300 font-bold text-3xl text-center">Recent Jobs</div>
            <div class="gap-4 grid grid-cols-1 md:grid-cols-3 mb-6">
                <!-- Job Listing 1: Software Engineer -->
                <div class="bg-white shadow-md p-4 rounded-lg job-card">
                    <h2 class="font-semibold text-xl">Software Engineer</h2>
                    <p class="mt-2 text-gray-700 text-lg">
                        We are seeking a skilled software engineer to develop
                        high-quality software solutions.
                    </p>
                    <ul class="bg-gray-100 my-4 p-4 rounded">
                        <li class="mb-2"><strong>Salary:</strong> $80,000</li>
                        <li class="mb-2">
                            <strong>Location:</strong> New York
                            <span
                                class="bg-blue-500 ml-2 px-2 py-1 rounded-full text-white text-xs">Local</span>
                        </li>
                        <li class="mb-2">
                            <strong>Tags:</strong> <span>Development</span>,
                            <span>Coding</span>
                        </li>
                    </ul>
                    <a href="details.html"
                        class="block bg-indigo-100 hover:bg-indigo-200 shadow-sm px-5 py-2.5 border rounded w-full font-medium text-indigo-700 text-base text-center">
                        Details
                    </a>
                </div>

                <!-- Job Listing 2: Marketing Specialist -->
                <div class="bg-white shadow-md p-4 rounded-lg job-card">
                    <h2 class="font-semibold text-xl">Marketing Specialist</h2>
                    <p class="mt-2 text-gray-700 text-lg">
                        We are looking for a Marketing Specialist to create and manage
                        marketing campaigns.
                    </p>
                    <ul class="bg-gray-100 my-4 p-4 rounded">
                        <li class="mb-2"><strong>Salary:</strong> $70,000</li>
                        <li class="mb-2">
                            <strong>Location:</strong> San Francisco
                            <span
                                class="bg-blue-500 ml-2 px-2 py-1 rounded-full text-white text-xs">Remote</span>
                        </li>
                        <li class="mb-2">
                            <strong>Tags:</strong> <span>Marketing</span>,
                            <span>Advertising</span>
                        </li>
                    </ul>
                    <a href="details.html"
                        class="block bg-indigo-100 hover:bg-indigo-200 shadow-sm px-5 py-2.5 border rounded w-full font-medium text-indigo-700 text-base text-center">
                        Details
                    </a>
                </div>

                <!-- Job Listing 3: Web Developer -->
                <div class="bg-white shadow-md p-4 rounded-lg job-card">
                    <h2 class="font-semibold text-xl">Web Developer</h2>
                    <p class="mt-2 text-gray-700 text-lg">
                        Join our team as a Web Developer and create amazing web
                        applications.
                    </p>
                    <ul class="bg-gray-100 my-4 p-4 rounded">
                        <li class="mb-2"><strong>Salary:</strong> $75,000</li>
                        <li class="mb-2">
                            <strong>Location:</strong> Los Angeles
                            <span
                                class="bg-blue-500 ml-2 px-2 py-1 rounded-full text-white text-xs">Local</span>
                        </li>
                        <li class="mb-2">
                            <strong>Tags:</strong> <span>Web Development</span>,
                            <span>Programming</span>
                        </li>
                    </ul>
                    <a href="details.html"
                        class="block bg-indigo-100 hover:bg-indigo-200 shadow-sm px-5 py-2.5 border rounded w-full font-medium text-indigo-700 text-base text-center">
                        Details
                    </a>
                </div>

                <!-- Job Listing 4: Data Analyst -->
                <div class="bg-white shadow-md p-4 rounded-lg job-card">
                    <h2 class="font-semibold text-xl">Data Analyst</h2>
                    <p class="mt-2 text-gray-700 text-lg">
                        We are hiring a Data Analyst to analyze and interpret data for
                        insights.
                    </p>
                    <ul class="bg-gray-100 my-4 p-4 rounded">
                        <li class="mb-2"><strong>Salary:</strong> $65,000</li>
                        <li class="mb-2">
                            <strong>Location:</strong> Chicago
                            <span
                                class="bg-blue-500 ml-2 px-2 py-1 rounded-full text-white text-xs">Remote</span>
                        </li>
                        <li class="mb-2">
                            <strong>Tags:</strong> <span>Data Analysis</span>,
                            <span>Statistics</span>
                        </li>
                    </ul>
                    <a href="details.html"
                        class="block bg-indigo-100 hover:bg-indigo-200 shadow-sm px-5 py-2.5 border rounded w-full font-medium text-indigo-700 text-base text-center">
                        Details
                    </a>
                </div>

                <!-- Job Listing 5: Graphic Designer -->
                <div class="bg-white shadow-md p-4 rounded-lg job-card">
                    <h2 class="font-semibold text-xl">Graphic Designer</h2>
                    <p class="mt-2 text-gray-700 text-lg">
                        Join our creative team as a Graphic Designer and bring ideas to
                        life.
                    </p>
                    <ul class="bg-gray-100 my-4 p-4 rounded">
                        <li class="mb-2"><strong>Salary:</strong> $60,000</li>
                        <li class="mb-2">
                            <strong>Location:</strong> Miami
                            <span
                                class="bg-blue-500 ml-2 px-2 py-1 rounded-full text-white text-xs">Local</span>
                        </li>
                        <li class="mb-2">
                            <strong>Tags:</strong> <span>Graphic Design</span>,
                            <span>Creative</span>
                        </li>
                    </ul>
                    <a href="details.html"
                        class="block bg-indigo-100 hover:bg-indigo-200 shadow-sm px-5 py-2.5 border rounded w-full font-medium text-indigo-700 text-base text-center">
                        Details
                    </a>
                </div>

                <div class="bg-white shadow-md p-4 rounded-lg job-card">
                    <h2 class="font-semibold text-xl">Data Scientist</h2>
                    <p class="mt-2 text-gray-700 text-lg">
                        We're looking for a Data Scientist to analyze complex data and
                        generate insights.
                    </p>
                    <ul class="bg-gray-100 my-4 p-4 rounded">
                        <li class="mb-2"><strong>Salary:</strong> $90,000</li>
                        <li class="mb-2">
                            <strong>Location:</strong> Seattle
                            <span
                                class="bg-blue-500 ml-2 px-2 py-1 rounded-full text-white text-xs">Remote</span>
                        </li>
                        <li class="mb-2">
                            <strong>Tags:</strong> <span>Data Science</span>,
                            <span>Machine Learning</span>
                        </li>
                    </ul>
                    <a href="details.html"
                        class="block bg-indigo-100 hover:bg-indigo-200 shadow-sm px-5 py-2.5 border rounded w-full font-medium text-indigo-700 text-base text-center">
                        Details
                    </a>
                </div>
            </div>
            <a href="listings.html" class="block text-xl text-center">
                <i class="fa-arrow-alt-circle-right fa"></i>
                Show All Jobs
            </a>
    </section>

    <!-- Bottom Banner -->
    <section class="mx-auto my-6 container">
        <div
            class="flex justify-between items-center bg-blue-800 p-4 rounded text-white">
            <div>
                <h2 class="font-semibold text-xl">Looking to hire?</h2>
                <p class="mt-2 text-gray-200 text-lg">
                    Post your job listing now and find the perfect candidate.
                </p>
            </div>
            <a
                href="post-job.html"
                class="bg-yellow-500 hover:bg-yellow-600 hover:shadow-md px-4 py-2 rounded text-black transition duration-300">
                <i class="fa fa-edit"></i> Post a Job
            </a>
        </div>
    </section>

</body>

</html>